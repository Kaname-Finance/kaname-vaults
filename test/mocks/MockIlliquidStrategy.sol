...
import {StrategyImpl, ERC20} from "../../src/StrategyImpl.sol";

contract MockIlliquidStrategy is StrategyImpl {
    ...
    function _harvestAndReport() internal override returns (uint256) {
        ...
        if (balance > 0) MockYieldSource(yieldSource).deposit(balance / 2);
    ...
    function _tend(uint256 /*_idle*/ ) internal override {
        ...
        if (balance > 0) MockYieldSource(yieldSource).withdraw(balance);
    }

    function availableDepositLimit(address _owner) public view override returns (uint256) {
        if (whitelist && !allowed[_owner]) return 0;
        else return super.availableDepositLimit(_owner);
    }
